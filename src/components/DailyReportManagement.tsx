  ...
  const filterPatients = () => {
    let filtered = patients.filter(patient => {
      const patientDate = new Date(
        (patient as Patient).admission_date || (patient as Consultation).created_at
      ...
      <div className="px-4 py-6 sm:px-0">
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
          <div className="px-4 py-5 sm:px-6">
            <h2 className="text-lg font-medium text-gray-900">Daily Reports</h2>
          </div>
          <div className="border-t border-gray-200">
            {dailyReports.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Report Content</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {dailyReports.map((report) => (
                      <tr key={report.report_id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{report.patients.patient_name}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{report.patient_id}</td>
                        <td className="px-6 py-4 text-sm text-gray-500">{report.report_content}</td>
              ...
              <p className="px-6 py-4 text-sm text-gray-500">No reports found for the selected date.</p>
            )}
          </div>
        </div>
